...
//        OLED_P8x16Str(1, 2, txt, u16BLACK, u16GREEN); //显示出库实际脉冲数，以便灵活调整
...
void TestEncoder(void)
{
    ...
    OLED_CLS();   //蓝色屏幕
    OLED_P8x16Str(0, 0, "Test Encoder");      //字符串显示
    EncInit();
    while (1)
    {
        /* 获取编码器值 */
...
void OutInGarageTft (uint8 inout, uint8 lr)// 测试用，有屏幕显示，不要冲突
{
    ...
    if (lr)           // 1右出入库
    {
        if (inout)    // 1右入库
        {
            ...
            while (RAllPulse > ps - 1500) //
            {
                ...
                OLED_P8x16Str(2, 4, txt); // 显示出库实际脉冲数，以便灵活调整
                delayms(10);
            }
            ...
            OLED_P8x16Str(2, 4, txt); // 显示出库实际脉冲数，以便灵活调整
            OLED_P8x16Str(2, 2, "!!WellDone!!"); // 入库完毕，永久停车
            while (1);                 // 入库完毕，永久停车
        }
        else  // 0右出库
        {
    ...
    else // 0：左出入库；
    {
        if (inout) // 1左入库
        {
            ...
            sprintf(txt, "Time:%d.%03ds", (sint16)(NowTime/1000),(sint16)(NowTime%1000));
            OLED_P8x16Str(2, 4, txt); // 显示出库实际脉冲数，以便灵活调整
            OLED_P8x16Str(2, 2, "!!WellDone!!", u16WHITE, u16RED); // 入库完毕，永久停车
            while (1) ;                 // 入库完毕，永久停车
        }
        else  // 0左出库
        {
...
void OutInGarage (uint8 inout, uint8 lr)
{
    ...
    if (lr)           // 1右出入库
    {
        if (inout)    // 1右入库
        {
            ...
            ServoCtrl(Servo_Center_Mid);  // 回中倒车
            MotorCtrl(2000);        // 左后倒车
//            MotorCtrl(1000, 1000);        // 左后倒车
            while (RAllPulse < ps + 2000) // 继续前进大约35cm
            {
                delayms(10);
            }
            MotorCtrl(-2500);      // 刹车
//            MotorCtrl(-500, -500);      // 刹车
            delayms(300);

            ps = RAllPulse;
            ServoCtrl(Servo_Right_Min);   // 舵机向右打死为入库做准备
            MotorCtrl(-3000);
//            MotorCtrl(-1000, -1000);
            while (RAllPulse > ps - 2000) // 从停车位出库，大约要512编码器2000个脉冲，龙邱512带方向编码器1米5790个脉冲
            {   // 右电机 母板上编码器1，小车后退为正值，并累加到出库为止
...
//            MotorCtrl(3000, 3000);
    ...
    else // 0：左出入库；
    {
        if (inout) // 1左入库
        {
            ...
            ps = RAllPulse;
            ServoCtrl(Servo_Left_Max);    // 舵机向右打死为入库做准备
            MotorCtrl(-3000);
